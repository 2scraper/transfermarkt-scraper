...
from output_writer import RemoteAPIError
import env_config
...
def get_fingerprint(api_key: str, *, tags: Optional[str] = None,
    ...
    try:
        ...
        raise RemoteAPIError("Fingerprint API request failed: %s" % _redact(exc)) from None

    if resp.status_code == 401:
        raise RemoteAPIError("Fingerprint API rejected the key (401). Note this is a "
    ...
    if resp.status_code == 400:
        ...
        if tags and any(sep in tags for sep in (",", "|", " ")):
            hint = (" — `tags` takes ONE OS-family value (Windows, "
                    ...
                    % tags)
        raise RemoteAPIError("Fingerprint API rejected the request (400): %s%s"
                           % (_redact(detail), hint))
    if resp.status_code == 429:
        raise RemoteAPIError("Fingerprint API rate limit hit (429). The per-minute cap "
    ...
    try:
        ...
        raise RemoteAPIError("Fingerprint API returned %s: %s"
...
def playwright_context_kwargs(fp: dict) -> dict:
    ...
    # A fingerprint that states a non-default DPI and gets a default-DPI
    # context contradicts itself: window.devicePixelRatio would read 1 while
    # the rest of the identity (screen size, UA) claims a HiDPI device. Only
    # set it when the API actually said one -- Playwright's own default (1)
    # is a reasonable fallback for a fingerprint that omits this field
    # entirely, and forcing a made-up value would be a guess wearing the
    # fingerprint's authority, the same mistake `locale` used to make below.
    dsf = screen.get("deviceScaleFactor") or screen.get("devicePixelRatio")
    if dsf:
        try:
            kwargs["device_scale_factor"] = float(dsf)
...
def main() -> int:
    ...
    # NOT defaulted from os.environ directly: that would only see an
    # already-EXPORTED variable, not one that lives in .env, and it would
    # send a copied .env.example placeholder straight to the API instead of
    # treating it as unset. env_config.apply() below fills this in with the
    ...
    p.add_argument("--key", default=None,
                   help="API key. Falls back to $TWOCAPTCHA_KEY via .env/"
                        "env_config, then to an exported environment "
                        "variable (safer than argv either way).")
    ...
    args = p.parse_args()
    # This CLI's own flag is spelled --key (not --twocaptcha-key like the
    # three engines), so the env mapping is spelled out explicitly -- same
    # pattern scraper_api_client.py uses for the same reason.
    env_config.apply(args, keys={"TWOCAPTCHA_KEY": "key"})

    if not args.key:
        logger.error("No API key. Pass --key, or better, put TWOCAPTCHA_KEY "
                     "in .env or export it.")
        return 2
