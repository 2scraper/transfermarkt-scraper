...
inline HTML fixtures, not a pytest suite -- see CONTRIBUTING.md for why. This
wraps it as one test rather than reimplementing its checks as pytest asserts,
so `pytest` and `python3 smoke_test.py` exercise the exact same code path
instead of two suites that can silently drift apart. Run
